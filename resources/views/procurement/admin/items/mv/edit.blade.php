...
<input type="hidden" id="edit_item_id" name="id" value="{{$item_master->id}}">
<div>
                        ...
                        <select name="item_category_id" class="form-select"
                            ...
                            @foreach ($categories as $key => $item)
                                <option value="{{ $item->id }}" 
                                    {{ $item_master->item_category_id == $item->id ? 'selected' : null }}>
                                    {{ $item->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-floating ">
                        <select name="item_subcategory_id" class="form-select"
                            ...
                            @foreach ($subcategories as $key => $item)
                                <option value="{{ $item->id }}"
                                    {{ $item_master->item_subcategory_id == $item->id ? 'selected' : null }}>
                                    {{ $item->name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="form-floating">
                    <select name="unit_type_id" class="form-select" id="edit_unit_type_id">
                        ...
                        @foreach ($unittypes as $key => $item)
                            <option value="{{ $item->id }}"
                                {{ $item_master->unit_type_id == $item->id ? 'selected' : null }}>
                                {{ $item->title }}
                            </option>
                        @endforeach
                    </select>
        ...
        <div class="col-12 gy-3">
            <div class="form-floating mb-3">
                <textarea class="form-control" name="description" id="add_item_description" placeholder="Product Description"
                    style="height: 100px">{{$item_master->description}}</textarea>
                <div class="invalid-feedback">
