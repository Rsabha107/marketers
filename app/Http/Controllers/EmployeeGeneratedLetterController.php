...
class EmployeeGeneratedLetterController extends Controller
{
    ...
    {
        //
        return view('tracki.employee.letter.generate.list');
    }
    ...
    public function create()
    {
        ...
        return view('tracki.employee.letter.generate.create', compact('letters', 'employees'));
    }
    ...
    public function list()
    {
        ...
        $op = $op->paginate(request("limit"))->through(function ($op) use ($user) {
            ...
            $profile_action =
                '<a href="' . route("tracki.employee.letter.generate.show", encrypt($op->id)) . '" target="_blank" class="btn-table btn-sm" title="View Template">' .
            ...
            $pdf_action =
                '<a href="' . route("tracki.employee.letter.generate.pdf", encrypt($op->id)) . '" class="btn btn-sm" data-table="employee_generated_letter_table" data-id="' .
                $op->id .
    ...
    public function show(string $id)
    {
        ...
        // $view = view(
        //     'tracki.employee.letter.mv.show',
        //     compact('letter')
        // )->render();

        // return response()->json(['view' => $view]);
        return view(
            'tracki.employee.letter.generate.show',
            compact('letter')
        );
    }
    ...
    public function edit(string $id)
    {
        ...
        return view('tracki.employee.letter.edit', compact('letter'));
    }
    ...
    public function update(Request $request)
    {
        ...
        return redirect()->route('tracki.employee.letter')->with($notification);
        // return response()->json(['error' => $error, 'message' => $message]);
    }
