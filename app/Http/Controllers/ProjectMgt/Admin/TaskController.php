...
class TaskController extends Controller
{
    ...
    public function list($id = null)
    {
        ...
        $ops = $ops->orderBy($sort, $order)->paginate(request('limit'))->through(function ($op) use ($statuses, $user) {
            ...
            if ($op->due_date  && $op->due_date < $mytime && $op->status->title != 'Completed') {
                $due_date_text_color = 'danger';
            } elseif ($op->status && $op->status->title == 'Completed') {
            ...
            $assign_to = $assign_to .   '<div class="avatar avatar-m  me-1">
                                            <a class="dropdown-toggle dropdown-caret-none d-inline-block"
                                                href="javascript:void(0);" id="edit_task_offcanv" data-table="task_table"
    ...
    public function store(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            if ($request->due_date) {
                $task->due_date = Carbon::createFromFormat('d/m/Y', $request->due_date)->toDateString();
            } else {
                $task->due_date = null;
            }
            // $task->start_time = $request->start_time;
            // $task->due_date =  $request->end_time;
            $task->budget_allocation = intval($request->budget_allocation);
            $task->description = $request->description;
            $task->status_id = intval($request->status_id);
            $task->project_id = $request->project_id;
            // $task->functional_area_id = $project->functional_area_id;
            // $task->color_id = $request->color_id;
            $task->actual_budget_allocated = intval($request->actual_budget_allocated);
    ...
    public function updateTask(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $error = false;
            $message = 'Task updated successfully to project ' . $task->project->name . '.';
        }
    ...
    public function getProjectAssigned($id)
    {
        $project = Project::findOrFail($id);
        $project_assigned = $project?->employees;

        return response()->json(['projectAssigned' => $project_assigned]);
    }
}
