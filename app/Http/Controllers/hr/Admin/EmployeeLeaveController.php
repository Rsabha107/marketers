...
use App\Mail\SendLeaveApprovalRequestMail;
...
class EmployeeLeaveController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            // lets send an email
            // log::info('reporting to id: '.$op->employees->reporting_to_id);
            $user = User::where('employee_id', $op->employees->reporting_to_id)->first();
            // dd($user);
            // log::info($user);
            $content = [
                'subject'   => 'Talent Nest, Leave approval for '.$op->employees->full_name,
                'name'      => $user->name,
                'body'      => 'Leave approval for '.$op->employees->full_name,
                'url'       => route('employee'),
            ];
    
            Mail::to($user->email)->queue(new SendLeaveApprovalRequestMail($content));
