...
class EmployeeTimeSheetController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('hr.admin.timesheet.list', compact('employees', 'employee_leave_statuses', 'months_name', 'employee_timesheets','timesheet_periods'));
    }
    ...
    public function entries($id)
    {
        ...
        // dd(FacadesRoute::currentRouteName());
        // dd(FacadesRequest::url());
        return view('hr.admin.timesheet.entries', compact('employee_timesheet'));
    }
    ...
    public function list($id = null)
    {
        ...
        $op = $op->paginate(request("limit"))->through(function ($op) use ($user) {
            ...
            // $profile_url = route('hr.admin.profile', $op->id);
            ...
            $go_to_route = route("hr.admin.timesheet.entries.add", encrypt($op->id));
            $pdf_route = route("hr.admin.timesheet.invoice.pdf", encrypt($op->id));

            $entries_icon_color = "success";

            if ($entries_count) {
                $go_to_route = route("hr.admin.timesheet.entries", encrypt($op->id));
    ...
    public function update(Request $request)
    {
        ...
        // Toastr::success('Has been add successfully :)','Success');
        // return redirect()->back()->with($notification);
        return Redirect::route('hr.admin.timesheet')->with($notification);
    }
