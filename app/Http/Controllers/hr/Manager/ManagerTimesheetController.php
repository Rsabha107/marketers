...
class ManagerTimesheetController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('hr.manager.timesheet.list', compact('employee_leave_statuses', 'months_name', 'timesheet_periods'));
    }
    ...
    public function list($id = null)
    {
        ...
        $op = $op->paginate(request("limit"))->through(function ($op) use ($user) {
            ...
            if ($entries_count) {
                if ($user->hasRole('Manager') && ($op->leave_statuses->title != 'Pending')) {
