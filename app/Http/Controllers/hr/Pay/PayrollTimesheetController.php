...
namespace App\Http\Controllers\hr\Pay;
...
class PayrollTimesheetController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('hr.payroll.timesheet.list', compact('employees', 'employee_leave_statuses','timesheet_periods'));
    }
    ...
    public function getEntries($id)
    {
        ...
        $view = view('/hr/payroll/timesheet/entries/mv/get', [
    ...
    public function missingTimesheet()
    {
        ...
        return view('hr.payroll.timesheet.missing.list', compact('directorate','entities','departments','functional'));
    }
    ...
    public function listMissingTimesheet()
    {
        ...
        $timesheet_period = (request()->timesheet_period) ? request()->timesheet_period : '';

        // $employees = Employee::whereDoesntHave('timesheets')->orderBy($sort, $order);
        $employees = Employee::whereDoesntHave('timesheets', function($query) use($timesheet_period){
            $query->where('timesheet_period', '=', $timesheet_period);
            // ->where('status_id','=', 11);
        })
    ...
    public function list($id = null)
    {
        ...
        $op = $op->paginate(request("limit"))->through(function ($op) use ($user) {
            ...
            // $profile_url = route('hr.employee.profile', $op->id);
    ...
    public function getEmpLeaveEditView($id)
    {
        ...
        $view = view('/hr/employee/leave/mv/edit', [
