...
class EmployeeTimeSheetEntryController extends Controller
{
    ...
    public function index($id)
    {
        ...
        $this->authorize('update', $employee_timesheets);
        ...
        return view('hr.emp.timesheet.entries.list', [
    ...
    public function add($id)
    {
        ...
        $this->authorize('update', $employee_timesheet);
        ...
        // dd(FacadesRoute::currentRouteName());
        // dd(FacadesRequest::url());
        return view('hr.emp.timesheet.entries.add', compact('employee_timesheet', 'months'));
    }
    ...
    public function list($id)
    {
        ...
        $ops = EmployeeTimeSheetEntry::orderBy($sort, $order);
        $ops = $ops->where('employee_timesheet_id', ($id));
        ...
        $ops = $ops->paginate(request("limit"))->through(function ($ops) use ($user) {

            $is_pending = ($ops->timesheet->leave_statuses->title == 'Pending') ? true : false;
            ...
            return [
                'id1' => '<div class="ms-3">' . $ops->id . '</div>',
                'id' => $ops->id,
                'calendar_day' => '<div class="ms-3">' . $ops->calendar_day . '</div>',
                'day_action' => '<div class="ms-1">' . $date_action_arr[$ops->day_action] . '</div>',
                'actions' => $actions,
    ...
    public function store(Request $request)
    {
        ...
        $isSalarySet = EmployeeSalary::where('employee_id', $request->employee_id)->count();


        if ($isSalarySet == 0){

            $notification = array(
                'message'       => 'You dont have a defined salary, please contact your HR representitave.',
                'alert-type'    => 'error'
            );
        ...
        // return redirect()->back()->with($notification);
        return Redirect::route('hr.emp.timesheet')->with($notification);
    }
    ...
    public function getEmpTimeSheetEditView($id)
    {
        ...
        $view = view('/hr/emp/timesheet/mv/edit', [
