...
class EmployeeTimeSheetController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('hr.emp.timesheet.list', compact('employees', 'employee_leave_statuses', 'months_name', 'employee_timesheets','timesheet_periods', 'employee_id'));
    }
    ...
    public function entries($id)
    {
        ...
        // dd(FacadesRoute::currentRouteName());
        // dd(FacadesRequest::url());
        return view('hr.emp.timesheet.entries', compact('employee_timesheet'));
    }
    ...
    public function list($id = null)
    {
        ...
        $op = EmployeeTimeSheet::where('employee_id', $user->employee_id)->orderBy($sort, $order);
        ...
        $op = $op->paginate(request("limit"))->through(function ($op) {
            ...
            // $profile_url = route('hr.emp.profile', $op->id);
            ...
            $go_to_route = route("hr.emp.timesheet.entries.add", encrypt($op->id));
            $pdf_route = route("hr.emp.timesheet.invoice.pdf", encrypt($op->id));

            $entries_icon_color = "success";

            if ($entries_count) {
                $go_to_route = route("hr.emp.timesheet.entries", encrypt($op->id));
            ...
            $profile_url = route('hr.emp.dashboard', encrypt($op->employees->id));
