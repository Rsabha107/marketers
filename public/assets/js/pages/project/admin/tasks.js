function getProjectAssigned(val) {
    // event_id = $(this).val();
    console.log("inside getProjectAssigned");
    console.log(val);
    project_id = val;
    $.ajax({
        url: "/projects/admin/task/getprojectassigned/" + project_id,
        type: "get",
        headers: {
        ...
        success: function (response) {
            ...
            $.each(response.projectAssigned, function (index, emp) {
                var option = $("<option>", {
                    value: emp.id,
                    text: emp.full_name,
                });
...
$(document).ready(function () {
    ...
    //   })
    $("#offcanvas-add-task-modal").on("hidden.bs.offcanvas", function (e) {
        $(this)
            ...
            $(".js-select-task-assign-multiple").val(null).trigger("change");
            $(".js-select-task-tags-multiple").val(null).trigger("change");
    });

    $("body").on("click", "#offcanvas-add-task", function () {
        console.log("inside #offcanvas-add-task");
        // $("#add_edit_form").get(0).reset()
        // console.log(window.choices.removeActiveItems())
        $("#cover-spin").show();
        $("#offcanvas-add-task-modal").offcanvas("show");
        $("#cover-spin").hide();
    ...
    //     console.log("Add");
    //     $("#add_task_modal_label").html("Add new taskxxx");
    //     $("#add_task_modal").modal("show");
    // });

    // $("#add_task_project_id").on("change", function(){
    ...
    $("body").on("click", "#edit_task_offcanv", function () {
        ...
        $.ajax({
            ...
            success: function (response) {
                g_response = response.view;
                $("#global-edit-task-content").empty("").append(g_response);

                $("#offcanvas-edit-task-modal").offcanvas("show");
                $("#cover-spin").hide();
            },
            error: function (xhr, ajaxOptions, thrownError) {
                console.log(xhr.status);
